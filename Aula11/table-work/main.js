...
const validateHobbies = e => {
  ...
  const hobbies = Array.from(checkedHobbies).map(hobbie =>
    hobbie.parentElement.textContent.trim()
  )

  let combinacao1 = ['Video Game', 'Cozinha'].every(hobbie =>
    hobbies.includes(hobbie)
  )
  let combinacao2 = ['Guitarra', 'Leitura'].every(hobbie =>
    hobbies.includes(hobbie)
  )
  let combinacao3 = ['Netflix', 'Costura'].every(hobbie =>
    hobbies.includes(hobbie)
  )

  if (combinacao1) {
    alert(
      'Vídeo Game e Cozinha não é uma combinação válida, por favor, marque outra opção!'
    )
    e.target.checked = false
    return false
  } else if (combinacao2) {
    alert(
      'Guitarra e Leitura não é uma combinação válida, por favor, marque outra opção!'
    )
    e.target.checked = false
    return false
  } else if (combinacao3) {
    alert(
      'Netflix e Costura não é uma combinação válida, por favor, marque outra opção!'
    )
    e.target.checked = false
    return false
  }
...
inputHobbiesRef.forEach(hobbie =>
  hobbie.addEventListener('change', enableBtnSubscription)
)
inputNacionalRef.forEach(nacionality =>
  nacionality.addEventListener('change', enableBtnSubscription)
)
